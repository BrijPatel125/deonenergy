 ...
 *
 * Used inside the main WP loop ($post is set). White card, #e5e5e5 border,
 * soft shadow. Featured image on top with the category badge overlaid on
 * it, then date, title, and excerpt below.
 *
 * @package Deon_Energy
 */
...
<article class="bg-white border border-deon-divider shadow-[0px_2px_10px_0px_rgba(0,0,0,0.06)] flex flex-col overflow-hidden h-full">
	<a href="<?php the_permalink(); ?>" class="relative block aspect-[16/10] overflow-hidden bg-deon-divider">
		...
		>
		<?php if ( $deon_cat ) : ?>
		<span class="absolute top-4 left-4 bg-white rounded-full px-4 py-1.5 font-sans font-medium text-[12px] leading-none text-deon-accent">
